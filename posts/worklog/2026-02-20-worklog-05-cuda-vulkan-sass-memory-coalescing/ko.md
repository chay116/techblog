...
- 핵심 주장: 이전 두 글에서 CUDA와 Vulkan이 단순 커널에서 거의 동일한 SASS를 생성하며, 유일한 차이가 디스크립터 바인딩 오버헤드라는 것을 확인했다. 벡터 덧셈 커널에서는 순수 대역폭 바운드여서 그것이 문제가 되지 않았다.
...
- 문제 정의: 메모리 코얼레싱 패턴이 CUDA와 Vulkan 경로에서 어떻게 보이는지 정리한다.
- 중요성: 대역폭 지배형 커널에서는 산술보다 메모리 접근 패턴이 성능을 결정한다.
- 성공 기준: 코얼레싱이 잘 된 경우와 아닌 경우의 해석 포인트를 정리한다.

# 3. Method and Setup

- Category/Track/Series: `worklog` / `api-language` / `gpu`
- 환경 요약: 하드웨어, 드라이버, 커널 구성, 분석 범위를 함께 기록한다.
- 재현 방법: AoS/SoA 및 접근 패턴별 차이를 반복 확인할 수 있게 남긴다.
