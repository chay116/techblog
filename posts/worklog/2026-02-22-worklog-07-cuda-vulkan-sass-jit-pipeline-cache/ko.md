...
- 핵심 주장: 지난 네 글에서 CUDA와 Vulkan이 NVIDIA GPU에서 거의 동일한 SASS를 생성한다는 것을 확인했다. 산술, 메모리 접근, 코얼레싱 동작 - 모두 동일. BDA를 사용하면 바인딩 모델까지 수렴한다.
...
- 문제 정의: 동일한 백엔드 컴파일러를 쓰는 두 경로에서 첫 실행 비용이 어디서 갈리는지 본다.
- 중요성: JIT 캐시와 파이프라인 캐시 전략은 사용자 체감 지연에 직접 연결된다.
- 성공 기준: cold/warm 경로 차이와 캐시 전략의 의미를 비교 관점으로 정리한다.

# 3. Method and Setup

- Category/Track/Series: `worklog` / `api-language` / `gpu`
- 환경 요약: 하드웨어, 드라이버, 캐시 전략, 측정 범위를 함께 기록한다.
- 재현 방법: 동일한 워크로드에서 cold/warm 실행 흐름을 반복 비교할 수 있게 남긴다.
