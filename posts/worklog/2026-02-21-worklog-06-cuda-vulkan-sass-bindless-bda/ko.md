...
- 핵심 주장: [워크로그 #04](/posts/worklog/2026-02-19-worklog-04-cuda-vulkan-sass-vector-add/)에서 CUDA와 Vulkan 사이의 유일한 구조적 SASS 차이를 확인했다: 디스크립터 간접 참조. CUDA는 상수 뱅크를 통해 버퍼 주소를 직접 전달한다. Vulkan은 글로벌 메모리의 디스크립터 테이블에서 가져오면서 추가 `LDG` 명령어가 발생한다.
...
- 문제 정의: bindless/BDA 접근이 CUDA raw pointer 모델과 얼마나 가까워질 수 있는지 본다.
- 중요성: 디스크립터 간접 참조를 줄이는 방식이 구조적 차이를 줄일 수 있다.
- 성공 기준: 주소 전달 방식에 따른 SASS 및 메모리 접근 차이를 설명한다.

# 3. Method and Setup

- Category/Track/Series: `worklog` / `api-language` / `gpu`
- 환경 요약: 하드웨어, 드라이버, 주소 전달 방식, 측정 범위를 함께 기록한다.
- 재현 방법: raw pointer, descriptor, BDA 경로를 반복 비교할 수 있게 남긴다.
